<template>
  <div class="flex flex-1 justify-center items-center">
    <div class="bg-cyan-700 px-10 py-6 rounded-2 flex flex-col gap-3">
...
const calculate = (type: mode) => {
  ...
  if (type === "first" && typeof data.value[type].value === "number") {
    data.value.second.value = parseFloat(
      (data.value[type].value! * getExchange()).toFixed(2)
    )
  }

  if (type === "second" && Number(data.value.second.value)) {
    data.value.first.value = parseFloat(
      (data.value[type].value! / getExchange()).toFixed(2)
    )
  }
}
...
const getExchange = () => {
  return exchangeRate.value[
    `${data.value.first.currency}-${data.value.second.currency}`
  ]
}
</script>
